...
class FileTexts extends Texts
{
    ...
    protected function buildText(string $locale, string $name): FileText
    {
        ...
        if (!file_exists($filename)) {
            throw new LangException('file-not-exist', [$filename]);
        }
    ...
    {
        if (!$this->directory) {
            throw new LangException('dir-key-not-found', [$this->getType()]);
        } else if (!file_exists($this->directory) && !mkdir($this->directory, 0775)) {
            throw new LangException('dir-create-error', [$this->directory]);
        }
        foreach ($locales as $locale) {
            $localePath = $this->directory . DIRECTORY_SEPARATOR . $locale;
            if (!file_exists($localePath)) {
                if (!mkdir($localePath, 0775)) {
                    throw new LangException('dir-create-error', [$localePath]);
                }
            }
            foreach ($this->getNames() as $name) {
                ...
                if (!file_exists($filename)) {
                    if (!file_put_contents($filename, "<?php\n\nreturn [\n\t'error' => ['message'],\n];\n")) {
                        throw new LangException('messages', 'file-create-error', [$filename]);
                    }
                    chmod($filename, 0664);
                }
            }
    ...
    private function getFileContent(string $filename, string $extension): array
    {
        if ($extension == 'php') {
            ...
            throw new LangException('file-unknown-type', [$extension, $filename]);
        }
    }
}